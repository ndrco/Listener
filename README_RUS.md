...
метаданными `application.id=speaker`, что делает приглушение и восстановление более
предсказуемыми. `sounddevice` остаётся доступным как явный бэкенд, а старые пути
подпроцессов сохраняются как резервные.
...
Когда OpenClaw передает длинный ответ, Listener ставит в очередь сегменты размером с
предложение. При использовании `speaker.tts_mode="persistent"` worker Piper остаётся
прогретым, и следующий сегмент можно синтезировать, пока текущий сегмент всё ещё
воспроизводится. Приглушение применяется один раз для активного запуска OpenClaw и
восстанавливается после последнего сегмента в очереди, вместо затухания между
предложениями.
