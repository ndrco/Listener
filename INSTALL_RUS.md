...
Основной `requirements.txt` также устанавливает `rutextnorm`. Listener использует его
для узкой нормализации русских числовых фрагментов перед Piper, VoxCPM2 или CosyVoice3;
отдельно устанавливать пакет в изолированные окружения нейронных моделей не нужно.
...
Для Speaker поместите модель Piper и её JSON-файл по путям
`models/ru_RU-irina-medium.onnx{,.json}`. Стандартная команда использует Piper из
собственной `.venv` Listener. Если модель ещё не готова, отключите Speaker при первом
запуске:
...
Для дополнительных бэкендов VoxCPM2 или Fun-CosyVoice3-0.5B вместо двух команд выше
установите проверенный единый профиль Python 3.12:

```bash
pip install -r requirements-unified-cu128-py312.txt
```

Модели сохраняют отдельные постоянные worker-процессы, но по умолчанию используют
окружение Python Listener. Поместите их ресурсы в `models/tts/`, а референсные WAV — в
`references/`; машинно-зависимые пути TTS в стандартном конфиге не нужны. Полное
руководство по установке, ресурсам и откату —
