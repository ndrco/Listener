...
## Пост-диаризация и нарезка сегментов (`audio.speaker`)

Для борьбы с длинными монологами без пауз поверх буфера добавлен модуль `audio.speaker.SpeakerDiarizer`. Он считывает готовые
сегменты `BufferedSpeechWriter`, прогоняет сырой PCM через `Resemblyzer` (модель хранится в `models/audio/resemblyzer`), ищет
смены голосов в скользящем окне и при необходимости делит исходный блок на несколько более коротких `SpeechSegment`. Каждый
подсегмент получает идентификатор динамического спикера (`speaker_cluster_id`), возможные `speaker_user_id`/`speaker_name` из
профиля или визуального трека, а также метаданные для публикации события `llm/speaker_phrase` в `llm_event_agent`.

Основные настройки находятся в секции `audio.speaker` (`cfg.audio.speaker`):

* `enabled` — включает модуль. Даже если файл весов не найден, diarizer продолжит работу на встроенной модели Resemblyzer и
  просто предупредит в логах.
* `model_path` — путь к `pretrained.pt` (по умолчанию `models/audio/resemblyzer/pretrained.pt`).
* `device` — устройство для инференса Resemblyzer (`cuda`, `cuda:0`, `cpu`).
* `queue_maxsize` — ограничение на внутреннюю очередь укороченных сегментов.
* `max_active_speakers` и `forget_after_s` — лимит одновременных спикеров и TTL их эмбеддингов, чтобы база не росла бесконечно.
* `match_threshold` — косинусное сходство для повторного использования существующего спикера; `profile_match_threshold` — аналогичный
  порог для сопоставления с сохранёнными профилями (`core.profiles`).
* `embedding_momentum` управляет сглаживанием эмбеддинга (0 — мгновенная подмена, 1 — среднее по истории).
* `partial_rate` и `min_coverage` задают параметры нарезки Resemblyzer (количество окон в секунду и минимальное покрытие последнего окна).
* `min_segment_ms` — минимальная длительность для выделенного куска (слишком короткие вставки будут слиты с соседями).
* `min_track_update_ms` — минимальная длительность реплики для обновления голосового эмбеддинга/кеша. Короткие куски проходят дальше в STT, но не влияют на модели спикеров и кеш сопоставления.
* `face_link_ttl_s` — сколько секунд учитывать события `face/talking`, чтобы привязать `track_id` к голосу.
* `profile_link_ttl_s` — окно, в течение которого сохранится последняя связка «трек ↔ голосовой embedding» для ручного создания
  профиля. Значение больше `face_link_ttl_s` позволяет нажать «Сохранить профиль» спустя паузу и всё ещё получить аудио-вектор.

После включения `AudioAgent` автоматически создаёт `SpeakerDiarizer`, а `WhisperStreamingTranscriber` начинает читать его очередь.
Благодаря этому STT получает уже размеченные по спикерам куски и пробрасывает всю метаинформацию дальше по конвейеру.
