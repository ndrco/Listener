...
файла. Каждая непустая строка считается запрещённым словом или фразой.

Матчинг выполняется в нормализованном виде:

- регистр игнорируется;
- знаки препинания и символы игнорируются;
- совпадение срабатывает не только на полную фразу, но и на вхождение внутри
  распознанного текста по границам слов.

Из-за этого в blacklist обычно достаточно хранить базовую форму без дублей,
а сам файл удобно держать в `config/blacklist.txt`:
...
Такие записи будут отсеивать и варианты вроде `1988, 1988, 1988`,
`Ну что, поехали.` или `А дальше... продолжение следует?`.
