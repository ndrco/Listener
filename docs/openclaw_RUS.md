...
отсутствовал в дельтах шлюза. При `speaker.tts_mode="persistent"` выбранный worker
остаётся прогретым. Готовые WAV Piper используют `speaker.playback.backend`, а
нейронный PCM — отдельный `speaker.playback.streaming_backend`. В Linux `auto`
предпочитает изолированный процесс `pacat` и использует его для всех сегментов одного
ответа OpenClaw.

Приглушение Speaker действует в пределах одного ответа OpenClaw. Для нейронного TTS оно
включается только после заполнения одноразового PCM-prebuffer и восстанавливается после
фактического слива последнего сегмента изолированным проигрывателем. Если после
прерывания или перебивания звук остаётся тихим,
...
Стандартная команда Piper использует собственную `.venv` Listener, а стандартная модель
находится по пути `models/ru_RU-irina-medium.onnx`. Поместите рядом соответствующий
JSON-файл либо установите `speaker.enabled=false`, пока локальная установка Piper не
будет готова.
